...
class EvaluationReport:
    ...
    def summary(self) -> str:
        ...
        lines = [
            ...
            f"K (retrieval cut): {self.k}",
            f"Eval Time        : {self.evaluation_time_s:.2f}s",
            "",
            "── 1. Retrieval ────────────────────────────────────────────",
            f"  Precision@{self.k:<3}  : {self.retrieval.get(f'precision_at_{self.k}', 'N/A')}",
            f"  Recall@{self.k:<6}   : {self.retrieval.get(f'recall_at_{self.k}', 'N/A')}",
            f"  F1               : {self.retrieval.get('f1', 'N/A')}",
            f"  MRR              : {self.retrieval.get('mrr', 'N/A')}",
            "",
            "── 2. Generation Quality ────────────────────────────────────",
            f"  BLEU             : {self.generation.get('bleu', 'N/A')}",
            f"  ROUGE-1 F1       : {self.generation.get('rouge', {}).get('rouge_1', {}).get('f1', 'N/A')}",
            f"  ROUGE-2 F1       : {self.generation.get('rouge', {}).get('rouge_2', {}).get('f1', 'N/A')}",
            f"  ROUGE-L F1       : {self.generation.get('rouge', {}).get('rouge_l', {}).get('f1', 'N/A')}",
            f"  BERTScore F1     : {self.generation.get('bertscore', {}).get('f1', 'N/A')}",
            "",
            "── 3. Faithfulness ──────────────────────────────────────────",
            f"  Label            : {self.faithfulness.get('label', 'N/A')}",
            f"  Score            : {self.faithfulness.get('score', 'N/A')}",
            f"  Method           : {self.faithfulness.get('method', 'N/A')}",
            "",
            "── 4. Relevance ─────────────────────────────────────────────",
            f"  Score (0–1)      : {self.relevance.get('score', 'N/A')}",
            f"  Raw Score (1–5)  : {self.relevance.get('raw_score', 'N/A')}",
            f"  Method           : {self.relevance.get('method', 'N/A')}",
            "",
            "── 5. Completeness ──────────────────────────────────────────",
            f"  Topic Coverage   : {self.completeness.get('topic_coverage', {}).get('coverage', 'N/A')}",
            f"  Entity Coverage  : {self.completeness.get('entity_coverage', {}).get('coverage', 'N/A')}",
            f"  Combined Score   : {self.completeness.get('combined_score', 'N/A')}",
            "",
            "── Overall Score ─────────────────────────────────────────────",
            f"  ⭐ Overall       : {self.overall_score()} / 1.0",
            "=" * 60,
        ]
        return "\n".join(lines)
